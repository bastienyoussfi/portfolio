...
export function useChat(agentSlug: string | null) {
  ...
  const sendMessage = useCallback(
    async (content: string) => {
      if (!content.trim() || !agentSlug) return
      ...
      try {
        ...
        const response = await fetch(`/api/chat?slug=${agentSlug}`, {
          method: 'POST',
        ...
        for await (const { event, data } of parseSSE(reader)) {
          ...
          switch (event) {
            case 'thinking_start':
              ...
              break
            case 'thinking_delta':
              dispatch({ type: 'APPEND_THINKING', text: data.text as string })
              break
            case 'thinking_end':
              dispatch({ type: 'SET_THINKING', isThinking: false })
              break
            case 'tool_use_start':
              dispatch({ type: 'ADD_TOOL_CALL', id: data.id as string, name: data.name as string })
              break
            case 'tool_use_end':
              dispatch({ type: 'UPDATE_TOOL_CALL', id: data.id as string, input: data.input as Record<string, unknown> })
              break
            case 'tool_result':
              dispatch({ type: 'TOOL_RESULT', toolUseId: data.tool_use_id as string, result: data.result })
              break
            case 'text_delta':
              dispatch({ type: 'APPEND_TEXT', text: data.text as string })
              break
            case 'done':
        ...
        dispatch({ type: 'SET_ERROR', message: 'Failed to connect. Please try again.' })
      }
    },
    [agentSlug],
  )
