...
>(({ title, images, stats, description, href, className }, ref) => {
  return (
    <a
      ...
      href={href}
      className={cn(
        "group relative block w-full cursor-pointer rounded-[var(--r)] border border-[var(--sep)] bg-[var(--card)] p-6 text-[var(--text-1)] shadow-[var(--shadow)] transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-[var(--shadow-hover)]",
        className
      )}
    ...
    >
      <div className="flex flex-col">
        ...
        <div className="relative mb-6 h-32">
          {images.map((src, index) => (
            <div
              key={index}
              className={cn(
                "absolute h-full w-[40%] overflow-hidden rounded-[var(--r-inner)] border-2 border-[var(--bg)] shadow-[var(--shadow)] transition-all duration-300 ease-in-out",
        ...
        {/* Stats Section */}
        <div className="mb-4 flex items-center space-x-4 text-sm text-[var(--text-3)]">
