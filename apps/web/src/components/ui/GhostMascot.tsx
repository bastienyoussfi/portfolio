...
function GhostMascot({ scale = 2, className, onClick }: GhostMascotProps) {
  const sz = BASE * scale
  const clipId = useId()
  ...
  return (
    <div
      className={className}
      onClick={onClick}
      style={{ width: sz, height: sz, display: "flex", alignItems: "center", justifyContent: "center" }}
    >
      <svg
        ref={svgRef}
      ...
      >
        ...
        <foreignObject width="231" height="289" clipPath={`url(#${clipId})`}>
          <div style={{ width: "100%", height: "100%" }}>
            <MeshGradient colors={COLORS} style={{ width: "100%", height: "100%" }} speed={1} />
          </div>
        </foreignObject>
...
export default memo(GhostMascot)
